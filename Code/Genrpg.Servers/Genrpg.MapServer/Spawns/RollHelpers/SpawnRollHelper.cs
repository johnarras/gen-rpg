...
namespace Genrpg.MapServer.Spawns.RollHelpers
{
    public class SpawnRollHelper : IRollHelper
    {
        ...
        public List<RewardList> Roll<SI>(IRandom rand, RollData rollData, SI item) where SI : ISpawnItem
        {
            ...
            if (st != null)
            {
                ...
                {
                    rollData.Depth++;
                    List<RewardList> list2 = _spawnService.Roll(rand, st.Items, rollData);
                    rollData.Depth--;
