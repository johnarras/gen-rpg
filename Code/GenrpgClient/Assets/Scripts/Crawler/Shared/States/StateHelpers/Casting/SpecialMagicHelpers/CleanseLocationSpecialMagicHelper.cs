...
{
    public class CleanseLocationSpecialMagicHelper : BaseSpecialMagicHelper
    {
        public override long GetKey() { return SpecialMagics.CleanseLocation; }
            ...
            SelectSpellAction action, CrawlerSpell spell, CrawlerSpellEffect effect, CancellationToken token)
        {
            ...
            for (int x = cx - radius; x <= cx + radius; x++)
            {
                ...
                for (int z = cz - radius; z <= cz + radius; z++)
                {
                    ...
                    {
                        _mapService.MarkCellCleansed(nx, nz);
                    }
                }
            }
