...
namespace Services.ProcGen
{
    ...
    public class ZoneGenService : IZoneGenService, IGameTokenService
    {
        ...
        public virtual Zone Generate(UnityGameState gs, long zoneId, long zoneTypeId, long extraSeed)
        {
            ...
            ZoneType zoneType = gs.data.GetGameData<ProcGenSettings>().GetZoneType(zoneTypeId);
            if (zoneType == null)
            {
                return null;
            }

            int maxLevel = gs.data.GetGameData<LevelSettings>().MaxLevel;
        ...
        protected virtual void SetTerrainConstants(UnityGameState gs, Map map, Zone zone, ZoneType zt, MyRandom rand)
        {
            ...
            if (zt.Textures != null)
            {
                List<TextureType> allTextures = gs.data.GetGameData<ProcGenSettings>().TextureTypes;
                if (allTextures == null)
                {
                    allTextures = new List<TextureType>();
                }

                if (gs.data.GetGameData<ProcGenSettings>().TextureChannels != null)
                {
                    for (int i = 0; i < gs.data.GetGameData<ProcGenSettings>().TextureChannels.Count; i++)
                    {
                        TextureChannel channel = gs.data.GetGameData<ProcGenSettings>().TextureChannels[i];
                        ...
                        if (currTextures.Count > 0)
                        {
                            ...
                            _reflectionService.SetObjectValue(zone, gs.data.GetGameData<ProcGenSettings>().TextureChannels[i].Name + "TextureTypeId", id);
                        }
                        else
                        {
        ...
        protected virtual void SetupFoliage(UnityGameState gs, Map map, Zone zone, ZoneType zt, MyRandom rand)
        {
            ...
            if (gs.data.GetGameData<ProcGenSettings>().PlantTypes == null)
            {
                gs.data.GetGameData<ProcGenSettings>().PlantTypes = new List<PlantType>();
            }

            if (gs.data.GetGameData<ProcGenSettings>().RockTypes == null)
            {
                gs.data.GetGameData<ProcGenSettings>().RockTypes = new List<RockType>();
            }

            if (gs.data.GetGameData<ProcGenSettings>().TreeTypes == null)
            {
                gs.data.GetGameData<ProcGenSettings>().TreeTypes = new List<TreeType>();
            }
            ...
            for (int times = 0; times < MapConstants.MaxGrass; times++)
            {
                ...
                {
                    PlantType ptype = gs.data.GetGameData<ProcGenSettings>().GetPlantType (pt.PlantTypeId);
                    if (ptype != null)
                    {
            ...
            {
                TreeType tt = gs.data.GetGameData<ProcGenSettings>().GetTreeType(ztt.TreeTypeId);
                if (tt == null)
                {
                    continue;
                }
        ...
        protected virtual void SetupMonsters(UnityGameState gs, Map map, Zone zone, ZoneType zoneType, MyRandom rand)
        {
            ...
            List<UnitType> units = gs.data.GetGameData<UnitSettings>().UnitTypes;
            if (units == null)
            {
                return;
            }
            ...
            foreach (SpawnItem si in spawnItems)
            {
                ...
                UnitType utype = gs.data.GetGameData<UnitSettings>().GetUnitType(si.EntityId);
                if (utype == null)
                {
                    continue;
                }
        ...
        private List<SpawnItem> CreateUnitListFromSpawnItems(UnityGameState gs, SpawnItem spawnItemIn, double chance, int depth)
        {
            ...
            {
                UnitType utype = gs.data.GetGameData<UnitSettings>().GetUnitType(spawnItemIn.EntityId);
                if (utype != null)
                {
            ...
            {
                SpawnTable spawnTable = gs.data.GetGameData<SpawnSettings>().GetSpawnTable(spawnItemIn.EntityId);
        ...
        public string GenerateZoneName(UnityGameState gs, long zoneTypeId, long extraSeed)
        {
                ...
                rand.NextDouble() < 0.30f)
            {
                ...
                NameList nl = gs.data.GetGameData<NameSettings>().GetNameList("ItemDoubleSuffix");
